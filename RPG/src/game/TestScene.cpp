...
bool TestScene::onInit (Importer* pkImporter, Renderer* pkRenderer)
{
	...
	m_kmyQuest = QuestFactory::GetInstance();
	m_kPj = m_kmyActor->CreateActor("Mariano" ,TYPE_PLAYER , TYPE_WARRIOR);
	m_kNpc = m_kmyActor->CreateActor("Troll" ,TYPE_NPC , TYPE_WARRIOR);
	...
	else
	{
		...
		m_kNpc->setSprite(m_pkAuxSprite);
		Actor2 = m_kNpc->getSprite();
		addEntity(Actor2);
		//addEntity(m_pkAuxSprite);
		m_pkAuxSprite = NULL;
	}
